...
import com.project.examportalbackend.models.response.QuizResultResponse;
...
//    }
    ...
    @GetMapping(value = "/all")
    public ResponseEntity<List<QuizResultResponse>> getQuizResults() {
        return ResponseEntity.ok(quizResultService.getQuizResults().stream().map(quizResult -> {
            User user = userRepository.findByUserId(quizResult.getUserId());
         //   Category category=categoryRepository.findByCatId(quizResult.getQuiz().getCategory().getCatId());
            return QuizResultResponse.builder()
                    .quizResId(quizResult.getQuizResId())
                    .userName(user.getFirstName() + " " + user.getLastName())
                    .attemptDatetime(quizResult.getAttemptDatetime())
                    .totalObtainedMarks(quizResult.getTotalObtainedMarks())
                    .quiz(quizResult.getQuiz())
                    .categoryName(quizResult.getQuiz().getCategory().getTitle()).build();
